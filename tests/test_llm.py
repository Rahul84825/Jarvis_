...
class TestGeminiClient(unittest.TestCase):
    def test_fallback_mode_no_api_key(self):
        ...
        self.assertIn("offline fallback mode", res_greet.lower())
        
        # Test generic fallback
        res_other = client.generate_response("what is the weather?")
        self.assertIn("gemini api key is missing", res_other.lower())
    ...
    def test_retry_on_failure(self, mock_sleep, mock_configure, mock_model_class):
        ...
        # Raise exception on first call, return response on second call
        ...
        mock_sleep.assert_called_once_with(1.0) # check first backoff sleep of 1.0s

    @patch('google.generativeai.GenerativeModel')
    @patch('google.generativeai.configure')
    @patch('time.sleep')
    def test_all_retries_fail(self, mock_sleep, mock_configure, mock_model_class):
        """Verifies that client returns connection error message if all retries fail."""
        mock_model = MagicMock()
        mock_chat = MagicMock()
        ...
        self.assertIn("trouble connecting to my cognitive services", res)
